...
Для планирования установки см. [требования к оборудованию]({{% ref "/docs/v1.2/install/hardware-requirements" %}}).
...
-   **Рекомендуется:** [загрузка в Talos Linux из другой Linux ОС с помощью `boot-to-talos`]({{% ref "/docs/v1.2/install/talos/boot-to-talos" %}}) —
...
-   [Установка с помощью временных DHCP- и PXE-серверов, запущенных в Docker-контейнерах]({{% ref "/docs/v1.2/install/talos/pxe" %}}) —
    требует дополнительной управляющей машины, но позволяет устанавливать систему сразу на несколько хостов.
-   [Установка с помощью ISO-образа]({{% ref "/docs/v1.2/install/talos/iso" %}}) — оптимальна для систем, которые умеют автоматизировать установку с ISO.

## Следующие шаги

-   После установки Talos Linux у вас будет набор узлов, готовых к следующему шагу:
    [установке и инициализации кластера Kubernetes]({{% ref "/docs/v1.2/install/kubernetes" %}}).
    
-   Прочитайте [обзор Talos Linux]({{% ref "/docs/v1.2/guides/talos" %}}), чтобы узнать, почему Talos Linux является оптимальным выбором ОС для Cozystack
