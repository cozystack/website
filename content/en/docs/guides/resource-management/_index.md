...
When deploying a service, a preset is defined in `resourcePreset` configuration variable, for example:
...
If both `resources` and `resourcePreset` are defined, `resourcePreset` is ignored.
...
### How Configuration Requests and Limits from Configuration
...
However, when upgraded, the configuration should be changed to the new form.
...
First of all, managed applications assume that the user doesn't need to have an in-depth knowledge of Kubernetes.
However, the explicit request and limit configuration was a "leaky abstraction" here, confusing and sometimes resulting in misconfigurations.

Another reason was that for hosting companies, running public clouds based on Cozystack, it's crucial to have a unified ratio through all the cloud.
This approach helps them ensure stable level of service and makes billing straightforward.
