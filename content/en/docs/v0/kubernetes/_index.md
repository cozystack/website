...
Cozystack deploys and manages Kubernetes-as-a-service as standalone applications within each tenantвЂ™s isolated environment.
...
-   `admin.conf` вЂ” The standard kubeconfig for accessing your new cluster.
    You can create additional Kubernetes users using this configuration.
-   `admin.svc` вЂ” Same token as `admin.conf`, but with the API server address set to the internal service name.
    Use it for applications running inside the cluster that need API access.
-   `super-admin.conf` вЂ” Similar to `admin.conf`, but with extended administrative permissions.
    Intended for troubleshooting and cluster maintenance tasks.
-   `super-admin.svc` вЂ” Same as `super-admin.conf`, but pointing to the internal API server address.
...
-   **Etcd Cluster**: A dedicated etcd cluster is deployed using Г†nix's [etcd-operator](https://github.com/aenix-io/etcd-operator).
...
- [Kamaji вЂ” Cluster API](https://kamaji.clastix.io/cluster-api/)
