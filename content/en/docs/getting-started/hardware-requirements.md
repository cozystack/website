...
- Primary disk should be at least 32 GB. Talos Linux uses it for the OS, images, and logs. It would be nice to have more.
...
- Machines should be able to use additional IPs, or an external load balancer should be available.
  In public clouds the use of the additional IPs typically needs to be allowed explicitly.
...
- **Three worker nodes** are a mandatory for running highly-available applications.
  If one of the three nodes becomes unavailable, either due to hardware failure or maintenance, you’ll be operating in a degraded state.
  While database clusters and replicated storage will continue functioning, it won’t be possible to start new database instances or create replicated volumes.
...
## Highly-Available applications notes
...
- Expect significant amount of horizontal, inter-node traffic inside clusters.
