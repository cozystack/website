...
class TestHtmlProtection(unittest.TestCase):
    ...
    def test_prose_comparison_is_not_swallowed(self):
        ...
        masked, store = lib.protect(text)
        # `<y` is a prose comparison, not a tag: it must reach the model unmasked.
        # A round-trip alone cannot catch an over-greedy tag pattern (any mask
        # round-trips), so assert the comparison stays visible and nothing was masked.
    ...
    def test_tag_inside_inline_code_is_protected_once(self):
        text = "use `<div>` here"
        masked, store = lib.protect(text)
        # The inline code is masked as one unit; the <div> inside must NOT be masked
        # again as a separate tag token (which a round-trip would not reveal).
        self.assertEqual(len(store), 1)
        self.assertTrue(next(iter(store)).startswith("§§INLINECODE_"))
        self.assertEqual(lib.restore(masked, store), text)
...
class TestTranscreatedPages(unittest.TestCase):
    ...
    def test_l10n_mode_reads_a_single_quoted_value(self):
        # `l10n: 'transcreate'` is the same YAML value as the bare form. A regex that
        # accepted only bare/double-quoted forms returned None here, sending a
        # hand-authored page back into the worklist to be silently re-translated.
        p = self._write("content/ru/q.md", "---\nl10n: 'transcreate'\n---\nbody\n")
        self.assertEqual(lib.l10n_mode(p), "transcreate")
...
class TestOrphanTranslations(unittest.TestCase):
    ...
    def test_superseded_orphans_above_floor_are_still_removed(self):
        # A version bump orphans a whole retired docs version at once. Those
        # superseded orphans must NOT count toward the moved-tree floor, or one
        # release freezes all orphan cleanup — including a genuinely deleted page.
        # Six superseded (> the floor if lumped) plus one deleted-source: the floor
        # sees only the one deleted, does not trip, and all seven are removed.
        for i in range(6):
            self._write(f"content/en/docs/v1.4/p{i}.md", "---\ntitle: x\n---\nhi\n")
            self._write(f"content/en/docs/v1.6/p{i}.md", "---\ntitle: x\n---\nhi\n")
            self._write(f"content/ru/docs/v1.4/p{i}.md", self.STAMPED)
        deleted = self._write("content/ru/docs/gone.md", self.STAMPED)
        orig_wl, orig_tc, orig_argv = lib.build_worklist, lib.find_transcreated, sys.argv
        try:
            lib.build_worklist = lambda *a, **k: []
            lib.find_transcreated = lambda *a, **k: []
            sys.argv = ["translate.py", "--lang", "ru"]
            translate.main()
        finally:
            lib.build_worklist, lib.find_transcreated, sys.argv = orig_wl, orig_tc, orig_argv
        for i in range(6):
            self.assertFalse(
                os.path.exists(os.path.join(self._tmp.name, f"content/ru/docs/v1.4/p{i}.md")),
                "superseded orphan was frozen by the floor")
        self.assertFalse(os.path.exists(deleted), "deleted-source orphan was not removed")
...
class TestRealContentScope(unittest.TestCase):
    ...
    def test_no_unknown_user_visible_frontmatter_keys(self):
        ...
        allowed = {
            ...
            # structural page keys (permalink override, body CSS class)
            "url", "body_class",
...
class TestPayloadProtocol(unittest.TestCase):
    ...
    def test_nested_placeholder_validates_only_top_level(self):
        # A shortcode nested inside inline code leaves only the OUTER token in the
        # masked body; the token-completeness check must consider only that
        # top-level token, not the inner one that never appears in the body — else
        # the page is rejected on every run and can never be translated.
        masked, store = lib.protect('a `{{< version-pin "cozystack_version" >}}` b')
        self.assertEqual(len(store), 2)  # inner SC nested inside the outer INLINECODE
        _, body = translate._split_payload_response(
...
class TestStaleReportCleanup(unittest.TestCase):
    ...
    def test_empty_worklist_still_reports_a_transcreated_page(self):
        # The mirror of the above: an empty worklist must NOT suppress the
        # transcreated-drift report. A hand-authored page whose English source
        # drifted still needs posting, or the report never fires on the steady-state
        # empty-worklist path.
        orig_wl, orig_orph, orig_tc, orig_argv = (
            lib.build_worklist, lib.find_orphan_translations, lib.find_transcreated, sys.argv)
        existed = os.path.exists(translate.REPORT_PATH)
        backup = open(translate.REPORT_PATH).read() if existed else None
        try:
            lib.build_worklist = lambda *a, **k: []
            lib.find_orphan_translations = lambda *a, **k: []
            lib.find_transcreated = lambda *a, **k: [("ru", "docs/x.md")]
            sys.argv = ["translate.py"]
            rc = translate.main()
            self.assertEqual(rc, 0)
            self.assertTrue(os.path.exists(translate.REPORT_PATH),
                            "transcreated report did not fire on the empty-worklist path")
            self.assertIn("docs/x.md", open(translate.REPORT_PATH).read())
        finally:
            (lib.build_worklist, lib.find_orphan_translations,
             lib.find_transcreated, sys.argv) = (orig_wl, orig_orph, orig_tc, orig_argv)
            if backup is not None:
                with open(translate.REPORT_PATH, "w", encoding="utf-8") as fh:
                    fh.write(backup)
            elif os.path.exists(translate.REPORT_PATH):
