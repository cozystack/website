...
TRANSLATABLE_FRONTMATTER_KEYS = (
    "title", "linkTitle", "description", "summary",
    # homepage use_cases[] cards (main's home use-cases feature): user-visible prose;
    # `link` stays structural. Reached by the recursive walk into the use_cases list.
    "situation", "outcome", "link_text",
)
...
def protect(text: str) -> tuple[str, dict[str, str]]:
    ...
    # 4. Raw HTML tags. Visible attribute values are exposed between placeholders
    # exactly as they are for shortcodes: masking a tag wholesale would leave alt
    # and title text in English on every localized page.
    ...
    # 5. Link destinations LAST, so URLs already inside code/shortcodes/HTML tags
    #    are covered by their own placeholders and are not double-masked. Link TEXT
