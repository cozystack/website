...
def _matches_any(rel: str, globs: list[str]) -> bool:
    return any(fnmatch.fnmatch(rel, g) or fnmatch.fnmatch(rel, f"**/{g}") for g in globs)
...
def is_hero(cfg: dict, rel: str) -> bool:
    return _matches_any(rel, cfg["model"].get("hero_globs", []))
